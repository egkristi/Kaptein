...
### External strategy review (2026-09) — engineering findings

An external reviewer analysed the project's **README, positioning, licensing, and market
context** (explicitly not the code). Most of its recommendations are product/GTM decisions
that belong to the maintainer, not this file — those are recorded as *open decisions* in
`ROADMAP.md`. Four of its observations are **verifiable engineering defects**, and one of
those is the best finding of the cycle. Evaluated and confirmed against the repo:

| # | Severity | Finding | Owner |
|---|----------|---------|-------|
| AF | **High** | **The MCP tool list is hand-maintained, not derived from the semantic layer — so the agent surface can silently drift from the human one.** `mcp.rs::tools()` is a literal `vec![Tool::new(…), …]` with **9 hand-written entries and zero references** to `actions_as_semantic`, `semantic::Action`, or any action graph. But `surface.rs`'s own doc comment states the contract: *"Headless and MCP are consumers of the **semantic layer** (they read the data plane and action graph…)"*, and ADR-0005's thesis is that no projection owns logic. Today, adding a view-model action does **not** expose it to agents, and removing one does not retract it — the two surfaces are maintained by hand and kept in sync by memory. This is the *same class* as findings U (guarded set enumerated by hand) and AC (redaction by convention): a contract asserted in prose that nothing enforces. It is also the class this project has already proven it cannot hold manually — three separate hand-maintained sets have drifted. **Fix:** generate the tool list from the action graph, or assert equivalence in a test. The side effect is a feature, not just hygiene: a new view-model action becomes agent-callable for free, which is the "domain layer is the product" thesis actually paying off. *Fixed: a canonical `semantic::MCP_TOOLS` registry now lives in the view-model semantic layer (the single source of truth for the governed agent tool set, ADR-0013 order), and a `tools_are_exactly_the_semantic_layer_registry` test asserts `mcp.rs::tools()` exposes exactly that set — no more, no less, same order — so a tool added/renamed on either side fails CI until the other follows.* | M1b.3 |
| AG | Medium | **The informer lifecycle has no property, fuzz, or chaos testing — and it is where four of this project's audit findings have landed.** `grep -rn 'proptest\|quickcheck\|arbitrary\|fuzz'` over `crates/` returns nothing; `informer.rs` has 9 example-based unit tests. Meanwhile findings **C** (LRU missing), **M** (cap unreachable), **N**/**Z** (`release`/`touch` uncalled) were *all* in this one subsystem, and **every one was found by reading code, not by a failing test**. That is an unusually strong empirical case for property testing a specific surface: the state machine is small (`register`/`touch`/`release`/`evict_idle` over a bounded map), the invariants are crisp and checkable (`live() <= max_watches`; a released key frees exactly one slot; the most-recently-touched key is never the eviction victim; register/release sequences never leak), and the bugs found so far are exactly the kind randomised sequences catch. Add a `proptest` (or a hand-rolled deterministic sequence fuzzer — no new dependency needed) over random operation sequences asserting those invariants. *Fixed: `randomized_sequences_preserve_the_lifecycle_invariants` — a deterministic, dependency-free, seeded LCG drives ≥10 000 random register/touch/release/evict sequences and asserts after every operation `live() <= max_watches`, touch/release agree on liveness, a release of a live key drops `live()` by exactly one, and eviction past the TTL drains to empty (no leak).* | M2.0c |
| AH | Low | **The README has no screenshot, GIF, or asciinema cast — for a terminal UI.** Verified: zero matches for `asciinema`/`.gif`/`.png`/`screenshot`. For a TUI the demo *is* the product; the comparable Rust/ratatui project (`fioletoven/b4n`) has attracted an order of magnitude more attention with materially less capability, largely on the strength of a README with screenshots and a keybinding table. Related and separately confirmed: the install section offers **two options both labelled "Recommended"** (`cargo install` at line 450, signed release at 458) followed by a paragraph explaining how to choose — a reader wants one path. *Partly fixed: the install section now leads with **one** "Recommended" path (signed release) and demotes `cargo install` to a clearly-labelled "Alternative" — a reader no longer has to choose between two Recommendeds. The screenshot/GIF/asciinema cast remains open (needs a TUI capture against a live cluster, tracked as a follow-up).* | Docs |
| AI | Low | **The stated MSRV *policy* and the actual MSRV contradict each other.** `docs/versioning.md` says the MSRV "is deliberately **not** 'latest stable' — it **lags to accommodate airgapped and distro toolchains**." The pin is `1.97.1`, which is effectively current; no distro-stable channel carries it, so the stated rationale is not achieved by the chosen value. Either the policy text is aspirational and should say so, or the pin should actually lag (which costs edition-2024 and `LazyLock` conveniences the codebase now uses). Worth settling explicitly because it interacts with the distribution question in `ROADMAP.md`'s open decisions. *Fixed: `docs/versioning.md` now states the policy honestly — the pin is 1.97.1 for edition-2024 + `LazyLock`, it is *not* currently lagging, and the lag rationale is a stated **intent** for when distro channels catch up rather than a claim the current value delivers.* | Docs |

**Claims evaluated and *not* adopted** — recorded so the same points don't recur:

- *"No measured performance data."* **Stale.** `benches/query.rs` gates p99 query latency, steady-state RSS, cold start, and fuzzy re-rank, and a `bench` CI job fails on regression. What is genuinely missing is narrower and now tracked in M1.8: the **kwok** synthetic-cluster harness and a **comparative** number against k9s (the README's comparison table does claim "the same speed", which is unbacked).
- *"M1.8 is buried behind other milestones."* **Stale** — it is a Phase 1 milestone and substantially landed.
- *"Publish `kaptein-viewmodel` on crates.io as a semver'd boundary."* **Already done** — `publish.yml` publishes all five crates on tag.
- *"Verify releases are published; add repo topics."* **Already done** — v0.32.0 is the latest published release and nine topics are set. (Stars/watchers/forks are indeed 0.)
- *"The roadmap contradicts the non-goals."* **Overstated.** M3b.2 reads "Image scan (Trivy/Grype)", i.e. shelling out — consistent with "no reimplemented scanners" — and Phase 3b is already explicitly *conditional* ("gated on Phase 3a finding users — an explicit stopping point, not a failure"). The legitimate residue is presentational: a reader of the README cannot easily tell integrate-from-implement, which is a framing fix rather than a scope contradiction.
- *"README is ~6,000 words."* It is **4,108**. The underlying point (it reads as a manifesto, not a landing page) stands regardless.
