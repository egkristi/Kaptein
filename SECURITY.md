...
- Releases are **signed** (cosign, keyless via the GitHub Actions OIDC identity) and ship
  ...
  file, with a Rekor transparency log entry per signature. The SBOM is itself
  cosign-signed, and each release carries **SLSA provenance** generated by
  `slsa-framework/slsa-github-generator` (the `provenance` job in
...
- **Verifying a release.** Each artifact ships a `.bundle` (a Rekor bundle containing the
  ...
  Then check the checksum: `sha256sum -c SHA256SUMS`. Verify SLSA provenance with
  [`slsa-verifier`](https://github.com/slsa-framework/slsa-verifier):

  ```bash
  slsa-verifier verify-artifact <file> \
    --provenance-path <file>.intoto.jsonl \
    --source-uri github.com/egkristi/Kaptein
  ```

  The SBOM is verified the same way as any artifact (`cosign verify-blob` against its
  `.cdx.json.bundle`).
