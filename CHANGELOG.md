...
- **M2.0b — the live integration-test tier now runs in CI.** A new `live` job in
  ...
  only as a locally-gated opt-in. The job is a **latest-three-minors conformance
  matrix** (v1.37 / v1.36 / v1.35, node images pinned by digest). The exec test's
  pod-readiness wait was also widened (15s → 60s) to tolerate a cold kind node's first
  image pull.
